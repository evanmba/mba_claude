...
// ─── Post log (sortable + computed ratios) ────────────────────────────────────

type PostSortKey = "date" | "title" | "reach" | "likes" | "shares" | "follows" | "shareRate" | "followRate" | "likeRate";
type SortDir = "asc" | "desc";

function SortTh({ label, col, activeCol, dir, onSort }: {
  label: string; col: PostSortKey; activeCol: PostSortKey | null; dir: SortDir; onSort: (c: PostSortKey) => void;
}) {
...
interface EnrichedPost extends IGPost {
  shareRate: number;   // shares / reach * 100
  followRate: number;  // follows / reach * 100
  likeRate: number;    // likes / reach * 100
}
            ...
            {sorted.map((p, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td className="py-2 pr-4" style={{ color: "var(--foreground)", maxWidth: "220px" }}>
