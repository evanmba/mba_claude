...
function HistoryTable({ rows }: { rows: PlatformMonthRow[] }) {
  const active = rows.filter((r) => r.totalPieces > 0);
        ...
        <table className="w-full text-xs min-w-max">
          <thead>
            <tr style={{ borderBottom: "1px solid var(--border)" }}>
              {["Month","IG","YT Long","YT Short","YT Posts","FB","TikTok","X","Pod","Email","Total Pieces"].map((h) => (
...
export function PlatformDashboard({ initialData, initialError, serverFetchedAt }: Props) {
  ...
  return (
    <>
      ...
      {/* KPI row */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6" style={{ maxWidth: "32rem" }}>
        <KPI
        ...
        />
        <KPI
          label="YTD Total Pieces"
          value={String(ytdPieces)}
          change={null}
        />
      </div>
